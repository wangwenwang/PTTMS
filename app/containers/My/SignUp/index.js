...
class SignUp extends Component {
  static navigationOptions = {
    ...
    componentDidMount() {
        this.props.dispatch(createAction('signUp/save')({ subLoading: false }))
    }
    ...
    sendCodeSubmit = () => {
        ...
        if (info.phone) {
            if (!is_mobile(info.phone)) {
                ...
                this.props.dispatch(createAction('signUp/sendCode')(info.phone))
            }
        } else {
            Toast.info('请输入手机号码', 1, null, true)
        }
    }

    // 重置isSend
    setSend = () => {
        this.props.dispatch(createAction('signUp/save')({ isSend: false }))
    }
    ...
    confirmCodeSubmit = () => {
        ...
        validateFields({ force: true }, (error) => {
            const info = getFieldsValue()
            if (!error) {
                this.props.dispatch(createAction('signUp/confirmCode')(info))
            }
        })
    }
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='always'>
              ...
              <WingBlank>
                  <Button
                      ...
                      // onClick={() => this.props.dispatch(NavigationActions.navigate({ routeName: 'SignUpNext', params: { phone: '18124047356' } }))}
